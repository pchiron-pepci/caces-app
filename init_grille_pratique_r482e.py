"""Peuple la grille d'evaluation pratique R.482 categorie E (base), mono-grille.
E = engin de transport (tombereau). 100 pts : Prise/16 + Conduite/42 + Travaux/30 + Fin/12.
Seuil par theme = bareme/2 (regle standard). Options PE et TEL facultatives (script separe).
Source : referentiel INRS R.482 A3/2, feuille Excel OTC 'Pratique E'.
Idempotent. E base = 1,0 UT.
"""
import os
...
THEMES = [
    ...
    ("Conduite et circulation", 42, [
        ("2", "Circuler a vide et en charge, en marche avant / en marche arriere, en ligne droite / en virage (a evaluer en continu durant la totalite des epreuves)", [
            ("Effectuer les manoeuvres avec souplesse et precision", 10),
    ...
    ("Travaux de base", 30, [
        ("3", None, [("Positionner le tombereau pour le chargement", 10)]),
        ("4", None, [("Effectuer un parcours test en montant les vitesses et en utilisant correctement les dispositifs de freinage", 10)]),
        ("5", None, [("Positionner le tombereau pour le dechargement et vider la benne", 10)]),
    ]),
...
for g in db.query(GrillePratique).filter(
        ...
        GrillePratique.categorie == "E",
        GrillePratique.type == "base").all():
    db.delete(g)
db.commit()

grille = GrillePratique(
    recommandation="R.482", categorie="E", type="base", code_option=None,
    libelle="Engins de transport (tombereau)",
...
print("[OK] Grille R.482 E creee (id=%s) - total %s (attendu 100)" % (grille.id, total))
