"""Peuple la grille d'evaluation pratique R.482 categorie D (base), mono-grille.
D = engin de compactage. 100 pts : Prise/16 + Conduite/42 + Travaux/30 + Fin/12.
Seuil par theme = bareme/2 (regle standard). Options PE et TEL facultatives (script separe).
Source : referentiel INRS R.482 A3/2, feuille Excel OTC 'Pratique D'.
Idempotent. D base = 1,0 UT.
"""
import os
...
THEMES = [
    ...
    ("Travaux de base", 30, [
        ("3", None, [("Effectuer le compactage d'une plate-forme", 30)]),
    ]),
...
grille = GrillePratique(
    recommandation="R.482", categorie="D", type="base", code_option=None,
    libelle="Engins de compactage",
...
print("[OK] Grille R.482 D creee (id=%s) - total %s (attendu 100)" % (grille.id, total))
