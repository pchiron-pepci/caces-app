...
| `Testeur` | `testeurs` | soft delete (`actif`) ; `etat` : actif/suspendu/sorti — modifiable avec PIN 1505 via `PUT /api/testeurs/{id}/etat`, défaut actif à la création ; docs PDF en base64 : `attestation_prevention_pdf/nom/date`, `visite_medicale_pdf/nom/visite_medicale_date`, `evaluation_pdf/nom/evaluation_date`, `autorisation_conduite_pdf/nom`, `carte_pdf/carte_nom_fichier` (legacy) ; `numero_nda` VARCHAR(50) nullable + `rcp_cle/rcp_nom/rcp_date` (RCP = assurance Responsabilité Civile Professionnelle du testeur, fichier R2) — ajoutés 2026-07-06, migration `migrate_testeur_nda_rcp.py` **à exécuter sur prod** ; champs UI ajoutés dans la modale (`templates/testeurs.html`, commit `08eae1a` : `#f-nda` texte + bloc RCP calqué sur les 4 documents existants avec `#modal-rcp-info/dl/date/file`, `#btn-suppr-rcp/upload-rcp`) mais **pas encore de JS ni de routes backend** — champs visuellement présents mais totalement inertes (aucun listener, aucun endpoint) tant que ces étapes ne sont pas livrées |
