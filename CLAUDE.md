...
- ~~**Règle métier préservée** : une saisie **neuve** exige toujours de lancer le chrono avant la 1re note.~~ **CADUC — voir ci-dessous : le garde-fou dur a finalement été retiré.**

**(1 bis) Garde-fou dur de notation SUPPRIMÉ (2026-07-30, décision assumée).** L'assouplissement ci-dessus ne suffisait pas : la modification restait bloquée à la reprise sur C1/B1. Le blocage au clic (`_actNote` → `e.preventDefault()` + toast « Lancez d'abord le compteur… ») a été **entièrement retiré** du handler.
**Pourquoi c'est sans risque — vérifié avant retrait** : le contrôle est **redondant**. `_compteursTempsIncomplets()` exige les 3 temps de tout compteur **entamé OU de toute section comportant des cases cochées** (`_compteurEntame(st, g.key) || !!avecNotes[g.key]`), et son consommateur **interdit la validation** par un `return` avec message bloquant (« Impossible de valider : temps manquants »). Un testeur qui note sans avoir lancé le chrono est donc **rattrapé à la validation**, qui reste le seul point de blocage — celui qui compte, puisque c'est lui qui décide de l'enregistrement.
**Ce qui subsiste** : le grisage de `_majSectionsVerrou` (opacity + bandeau ambre) comme **simple indice visuel**, sans effet fonctionnel.
**RÈGLE : ne jamais rétablir de blocage de notation dans le handler de clic.** Un commentaire le rappelle à l'emplacement exact du bloc retiré. Le contrôle des temps appartient à la validation, pas à la saisie.
...
**Commits :** migration colonne (6952f2f), modèle ORM (33787fc), upload _H/_F (a5b7816), exposition audio_f (c886bd3), front tablette (d5aa1b7), projection salle (73ed447), script génération audio (0600544), auto-migration démarrage (bd4c034). **Fix (ade232b) :** le 5/6 câblait `q.audio_f` dans le JS projection mais `page_projection_theorie` ne l'exposait pas dans `questions_flat` → bouton Femme inopérant en salle (retombait sur voix H). Corrigé en ajoutant `"audio_f"` au dict. **Fix jumeau (64462e0) :** même oubli côté tablette — le JS reconstruisait l'objet `questions` sans recopier `audio_f` → switch Femme inopérant à la réécoute ; corrigé en ajoutant `audio_f: q.audio_f || null`. Règle : chaque consommateur front doit reforwarder explicitement les champs de la payload (`get_questions_phase2` expose, mais `questions_flat` projection ET l'objet JS tablette doivent chacun recopier).
...
### ✅ Visibilité terrain par session (chantier complet, 2026-07-08)

Décider manuellement, depuis le back-office, quelles personnes affectées voient une session dans leur espace **terrain**. Décoché par défaut : une personne affectée ne voit rien tant qu'elle n'est pas explicitement cochée.

- **Modèle / stockage (commit 4f8a56e) :** table `session_visibilite (session_id, user_id)` + contrainte unique `uq_session_visibilite` (`app/models/session_visibilite.py`). 1 ligne = cette personne voit cette session ; absence de ligne = ne voit pas. Migration au démarrage dans `_run_startup_migrations` (`CREATE TABLE IF NOT EXISTS`, PostgreSQL prod ; SQLite local tolère `SERIAL`).
- **Routes API (`app/routers/sessions.py`, prefix `/api/sessions`) :** `GET /{id}/visibilite` renvoie `[{user_id, nom, prenom, visible}]` — personnes affectées + état coché ; `PUT /{id}/visibilite` remplace l'ensemble coché (ne coche que des personnes réellement affectées). Gardes `not in ("admin", "utilisateur")`.
- **Source des personnes affectées (`_personnes_affectees_session`) :** formateurs via `AffectationFormation.user_id` **et** testeurs via `AffectationTest.user_id` (commit 982a492 : c'est la vraie source — `JourTest.testeur_id` est souvent `None`, ce qui vidait la modale). Les deux sont déjà des `user_id`, dédoublonnés.
- **Filtre terrain (commit 2405e8f, `page_sessions` dans `main.py`) :** un utilisateur `terrain` ne voit que les sessions où il est coché (`Session.id.in_(_sids_visibles)`) ; aucune cochée → `.filter(False)` (→ `WHERE false`) → liste vide + message dédié dans `sessions.html` (`user_role == 'terrain' and sessions|length == 0`). Rôles `admin`/`utilisateur` : voient tout (inchangé).
- **UI back-office (commits e54e70c, bf83c39, 6602dd9) :** « œil » 👁️ réservé admin/utilisateur ouvrant une modale (`modal-visibilite`) — liste des personnes avec cases, Tout cocher/décocher, Enregistrer → PUT (`static/js/session_detail.js`, event delegation sur `document`). Emplacement final : icône dans le **header de page** (`{% block title_extra %}` ajouté à `base.html`, vide ailleurs) à côté du titre, 26px.
...
### ⏳ À VALIDER EN PROD (non marqué terminé) : colonnes CDT/Hors/% hors onglet Tests & CDT (commit cf614ba)
...
### ⏳ À VALIDER EN PROD (non marqué terminé) : périmètre NORYX du module stats (commit à venir)

- **Périmètre du module stats** = tests réellement **produits par NORYX** sur la période. Exclusion des populations hors-périmètre dans `app/routers/statistiques.py`.
- **Exclusion REPRISE — partout (4 fonctions)** : `_stats_pratique_famille_cat`, `_stats_theorie_famille`, `_stats_par_testeur` (qth + qpr) → filtre `(SessionModel.type != "reprise") | (SessionModel.type.is_(None))` (le **OR NULL obligatoire** : anciennes sessions à `type` NULL). Pour `_stats_caces_delivres` la reprise s'exclut via `(CacesObtenu.ancien_numero.is_(None)) | (== "")` (un CACES repris = `ancien_numero` rempli). **Motif** : la reprise d'historique est **partielle par construction** (l'admin ne ressaisit que les CACES utiles à une extension) → population non exhaustive, inexploitable en statistique. L'antériorité relevait de la compta pré-NORYX de l'OTC.
- **Exclusion SOUS-TRAITANCE — pratique + colonne CACES uniquement**, PAS dans l'onglet Testeurs. Helper `_cles_sous_traitance(db)` = ensemble des `(stagiaire_id, session_id, categorie)` des `CacesObtenu.sous_traitance == True` ; dans `_stats_pratique_famille_cat` on **skippe** l'épreuve si sa clé y figure, **AVANT** tout incrément. **Asymétrie assumée** : Testeurs compte la sous-traitance (« épreuve réellement passée devant le testeur ») vs Tests & CDT (« qu'a produit l'organisme ») → écart possible entre les 2 onglets, documenté dans les mentions de périmètre en tête d'onglet.
- **Clé de corrélation sous-traitance** = `(stagiaire_id, session_id, categorie)`, fiable car `UniqueConstraint uq_caces_obtenu` (`caces_obtenu.py:32`).
