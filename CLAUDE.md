...
| `CacesObtenu` | `caces_obtenus` | statut : `a_valider`/`valide`/`annule` ; `numero_ordre` (Integer unique, attribué à la validation) ; UNIQUE(stagiaire_id, session_id, categorie) ; routes : GET `/api/caces-obtenus/a-valider` (sync + liste), GET `/api/caces-obtenus/valides`, POST `/api/caces-obtenus/valider/{id}?pin=` (attribue numéro incrémental, bouton "📜 Émettre le CACES®"), POST `/api/caces-obtenus/remettre-en-revision/{id}?pin=` (statut→`annule`, conserve numero_ordre, bouton "↩ Révision") ; au prochain appel `/a-valider` les records `annule` repassent automatiquement en `a_valider` si les données source sont toujours valides ; service `app/services/caces_obtenus.py` → `calculer_et_synchroniser(db)` |
