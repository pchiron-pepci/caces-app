...
8. **Retrait candidat d'un jour** : bloqué (400) si des `SessionEpreuve` existent pour ce candidat/jour (l'utilisateur doit d'abord supprimer les résultats via "Annuler le résultat") ; sinon supprime `ResultatTheorie` liés au jour puis hard delete `JourTestCandidat`
