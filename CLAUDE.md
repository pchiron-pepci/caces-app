...
### ✅ Grille théorique R.486 — grille 1/5 (100 questions) — script créé (2026-07-06, commit `3a09861`)

**Fichier :** `init_questions_r486.py` (idempotent — supprime puis recrée toutes les grilles `famille="R486"` avant réinsertion, purge aussi les `UtilisationGrille` liées). Structure 4 thèmes fixes (14/26/54/6 = 100 questions), poids uniforme 1 pt/question (`POINTS_THEME`). Convention image/audio `R486_G{grille}_T{theme}_Q{numero}` (underscores), identique au pattern déjà en place pour les autres familles (`upload.py`, `split("_")`).

**⚠️ Livraison partielle assumée, pas un oubli :** seule la grille 1 est renseignée (`GRILLES_R486 = {1: {...}}`) — le commentaire du script et le message de commit indiquent explicitement "grilles 2-5 à suivre". Tant que les grilles 2 à 5 ne sont pas ajoutées, le tirage Phase 2 pour R.486 ne peut piocher que dans cette unique grille — fonctionnellement limitant mais pas bloquant (mécanisme de tirage inchangé, il traite juste un pool réduit à 1 grille pour l'instant).

**Vérifications effectuées avant commit (hors exécution en base) :** extraction programmatique de `GRILLES_R486` pour confirmer, par thème : séquence `numero_question` 1..N sans trou ni doublon, et total exact 100 questions — conforme sur les 4 thèmes (14/26/54/6). Pas de vérification possible du couple question/réponse par rapport à la source INRS (contenu métier, hors de portée d'une vérification automatisée).
