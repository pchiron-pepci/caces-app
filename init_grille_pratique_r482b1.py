"""Peuple la grille d'evaluation pratique R.482 categorie B1 (base).
Source : referentiel INRS R.482 A3/2, cale sur la feuille Excel OTC 'Pratique B1'.
Idempotent : supprime puis recree la grille B1 base.
B1 base = 1,0 UT. Options Porte-Engins et Telecommande = facultatives (script separe).
"""
import os
...
THEMES = [
    ...
    ("Conduite et circulation", 24, [
        ("2", "Circuler a vide et en charge, en marche avant / en marche arriere, en ligne droite / en virage (a evaluer en continu durant la totalite des epreuves)", [
            ("Circuler a vide et en charge, en marche avant / en marche arriere, en ligne droite / en virage", 6),
            ...
            ("Utiliser correctement les dispositifs de freinage", 3),
    ...
    ("Travaux de base", 30, [
        ("3", None, [
            ("Charger une unite de transport", 10),
        ]),
        ("4", None, [
            ("Effectuer une operation de deblai / remblai avec mise en stock", 10),
        ]),
        ("5", None, [
            ("Realiser une tranchee", 10),
        ]),
    ]),
    ("Operation de levage", 18, [
        ("6", None, [
            ...
            ("Effectuer l'operation de levage (prise et depose d'une charge au sol)", 6),
        ]),
    ]),
...
# Suppression idempotente de la grille B1 base existante
anciennes = db.query(GrillePratique).filter(
    GrillePratique.recommandation == "R.482",
    GrillePratique.categorie == "B1",
...
grille = GrillePratique(
    recommandation="R.482", categorie="B1", type="base", code_option=None,
    libelle="Engins d'extraction et/ou de chargement a deplacement sequentiel",
...
print("[OK] Grille R.482 B1 base creee (id=%s)" % grille.id)
