"""Peuple la grille d'evaluation pratique R.482 categorie C3 (base), mono-grille.
C3 = engins de nivellement a deplacement alternatif (niveleuse). 100 pts : Prise/16 + Conduite/42 + Travaux/30 + Fin/12.
Seuil par theme = bareme/2 (regle standard). Options PE et TEL facultatives (script separe).
Source : referentiel INRS R.482 A3/2, feuille Excel OTC 'Pratique C3'.
Idempotent. C3 base = 1,0 UT.
"""
import os
...
THEMES = [
    ...
    ("Travaux de base", 30, [
        ...
        ("4", None, [("Realiser le reglage d'un talus ou un fosse avec la lame deportee", 15)]),
    ]),
...
for g in db.query(GrillePratique).filter(
        ...
        GrillePratique.categorie == "C3",
        GrillePratique.type == "base").all():
    db.delete(g)
db.commit()

grille = GrillePratique(
    recommandation="R.482", categorie="C3", type="base", code_option=None,
    libelle="Engins de nivellement a deplacement alternatif",
...
print("[OK] Grille R.482 C3 creee (id=%s) - total %s (attendu 100)" % (grille.id, total))
