"""Peuple la grille d'evaluation pratique R.482 categorie B3 (base), mono-grille.
B3 = engins rail-route a deplacement sequentiel. 100 pts :
Prise/17 (specifique rail : accessoires levage, mecanismes montee/descente lorries, groupe secours)
+ Conduite/23 (mode route/rail, manoeuvres enraillement/deraillement)
+ Travaux/30 (3 PE : charger, deblai-remblai, tranchee)
+ Levage/18 (4 items) + Fin/12 (6 items).
Seuil par theme = bareme/2. Options PE et TEL facultatives (script separe).
Source : referentiel INRS R.482 (grille officielle B3). Idempotent. B3 base = 1,0 UT.
"""
import os
...
THEMES = [
    ("Prise de poste et mise en service", 17, [
        ("1", "Verifier la presence et la validite des documents reglementaires suivants, et savoir les exploiter :", [
            ...
            ("Verifier le bon fonctionnement des dispositifs de securite", 1),
            ("Verifier la presence et l'etat des accessoires de levage", 1),
            ("Verifier le bon fonctionnement des mecanismes de montee / descente des lorries", 1),
            ("Verifier le bon fonctionnement du groupe de secours", 1),
            ("Identifier la position de l'issue de secours et savoir expliquer sa mise en oeuvre", 1),
        ]),
    ]),
    ("Conduite et circulation", 23, [
        ("2", "Circuler a vide et en charge, en marche avant / en marche arriere, en ligne droite / en virage, en mode route / en mode rail, selon le parcours de test defini (a evaluer en continu durant la totalite des epreuves)", [
            ("Circuler a vide et en charge, en marche avant / en marche arriere, en ligne droite / en virage, en mode route / en mode rail", 5),
            ...
            ("Maitriser la selection des vitesses", 1),
            ("Utiliser correctement les dispositifs de freinage", 2),
            ("Effectuer en securite les manoeuvres de montee / descente de banquette et d'enraillement / deraillement", 3),
            ("Recourir de facon appropriee aux aides a la conduite disponibles", 3),
        ]),
    ]),
    ("Travaux de base", 30, [
        ("3", None, [("Charger une unite de transport", 10)]),
        ("4", None, [("Effectuer une operation de deblai / remblai avec mise en stock", 10)]),
        ("5", None, [("Realiser une tranchee", 10)]),
    ]),
    ("Operation de levage", 18, [
        ("6", None, [
            ...
            ("Effectuer l'operation de levage (prise, circulation en charge et depose d'une charge au sol)", 6),
        ]),
    ]),
...
for g in db.query(GrillePratique).filter(
        ...
        GrillePratique.categorie == "B3",
        GrillePratique.type == "base").all():
    db.delete(g)
db.commit()

grille = GrillePratique(
    recommandation="R.482", categorie="B3", type="base", code_option=None,
    libelle="Engins rail-route a deplacement sequentiel",
...
print("[OK] Grille R.482 B3 creee (id=%s) - total %s (attendu 100)" % (grille.id, total))
