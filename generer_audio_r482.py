...
Textes lus depuis init_questions_r482.py (dict GRILLES_R482), SSML genere par regex.
Nommage : R482_G{grille}_T{theme}_Q{numero}_{H|F}.mp3
Echantillons d'intro : R482_ECHANTILLON_{H|F}.mp3

USAGE (dans un environnement avec credentials AWS, ex. CloudShell) :
  pip3 install boto3
  python3 generer_audio_r482.py                 -> 1000 MP3 (H+F) + 2 echantillons
  python3 generer_audio_r482.py H               -> voix homme uniquement
  python3 generer_audio_r482.py F --force       -> regenere voix femme
  python3 generer_audio_r482.py G1 T3 Q47 F --force  -> une question, voix femme
"""
import os, re, sys, ast, html, boto3

_src = open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "init_questions_r482.py"), encoding="utf-8").read()
_tree = ast.parse(_src)
GRILLES_R482 = None
for _node in _tree.body:
    if isinstance(_node, ast.Assign):
        for _t in _node.targets:
            if isinstance(_t, ast.Name) and _t.id == "GRILLES_R482":
                GRILLES_R482 = ast.literal_eval(_node.value)
if GRILLES_R482 is None:
    print("[ERREUR] GRILLES_R482 introuvable"); sys.exit(1)
...
def main():
    ...
    out_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "audio_r482")
    ...
    genere = saute = 0
    for gnum, themes in GRILLES_R482.items():
        ...
        for tnum, questions in themes.items():
            ...
            for (qnum, texte, _r) in questions:
                ...
                for v in voix_a_faire:
                    nom = "R482_G%d_T%d_Q%d_%s.mp3" % (gnum, tnum, qnum, v)
                    chemin = os.path.join(out_dir, nom)
